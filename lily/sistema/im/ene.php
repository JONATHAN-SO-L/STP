...
<?php
  ...
  // Se verifica la existencia de registros en la DDBB
    $resultado = mysqli_query($conexion,"SELECT EXISTS (SELECT * FROM enero_l);");
    $row=mysqli_fetch_row($resultado);

      if ($row[0]=="1") {

        $resultado = mysqli_query($conexion, "SELECT * FROM enero_l");
      ...
      echo "
              <div class='container'> 
                ...
                <div class='form-group'>
                  <center>
                  <a href='/stp/lily/sistema/im/mod1.php' class='btn active btn-warning' style='color:white;'>MODIFICAR INFORME</a>
                  </center>
                </div>
              </div>
            <br><br>";}
...
// Se insertan los datos en la DDBB
        $conexion->query("INSERT INTO enero_l (publicaciones, videos, horas, revisitas, cursos, comentarios) VALUES ('$pubs', '$videos', '$hours', '$revisit', '$courses', '$comments')");

// Mensaje de registro exitoso
        echo "<script>alert('INFORME REGISTRADO')</script>";

// Se redirecciona al index del apartado mediante HTML5
        echo "<meta HTTP-EQUIV='REFRESH' CONTENT='0;URL=/stp/lily/sistema/im'>";
