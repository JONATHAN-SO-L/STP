...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
  ...
  <form class="form-inline float-lg-right" id="volver">
    <a href="/stp/soledad/sistema/im" class="btn btn-danger btn-sm">VOLVER</a>
  </form>
...
<?php
  ...
  include "../../funciones/conexion.php"; // Se invluye la conexión con la DDBB

  // Se verifica la existencia de registros en la DDBB
    $resultado = mysqli_query($conexion,"SELECT EXISTS (SELECT * FROM enero_s);");
    $row=mysqli_fetch_row($resultado);

      if ($row[0]=="1") {

        $resultado = mysqli_query($conexion, "SELECT * FROM enero_s");

// Mientras haya información la va a mostrar en un arreglo de filas
      while ($enero = mysqli_fetch_array($resultado)) {

// Mensaje de confirmación de datos existentes
      echo "
            <br><br>
              <div class='container'>
...
// Tabla que muestra los registros en la DDBB
      echo "
              <div class='container'> 
                  <table class='table table-dark table-hover'>
                    <tr>
                      <td><center>PUBLICACIONES</center></td>
                      <td><center><strong>".$enero['publicaciones']."</strong></center></td>
                    </tr>
                    <tr>
                      <td><center>VIDEOS</center></td>
                      <td><center><strong>".$enero['videos']."</strong></center></td>
                    </tr>
                    <tr>
                      <td><center>HORAS</center></td>
                      <td><center><strong>".$enero['horas']."</strong></center></td>
                    </tr>
                    <tr>
                      <td><center>REVISITAS</center></td>
                      <td><center><strong>".$enero['revisitas']."</strong></center></td>
                    </tr>
                    <tr>
                      <td><center>NÚMERO DE <i> DIFERENTES </i><br> CURSOS BÍBLICOS DIRIGIDOS</center></td>
                      <td><center><strong>".$enero['cursos']."</strong></center></td>
                    </tr>
                    <tr>
                      <td><center>COMENTARIOS</center></td>
                      <td><center><strong>".$enero['comentarios']."</strong></center></td>
                    </tr>
                </table>
                <div class='form-group'>
                  <center>
                  <a href='/stp/soledad/sistema/im/mod1.php' class='btn active btn-warning' style='color:white;'>MODIFICAR INFORME</a>
                  </center>
                </div>
              </div>
            <br><br>";}
...
// Fomrulario de inserción de datos
        echo '
  <div class="container">
    <form class="form-inline" id="informe" method="POST" action="#"><br>
      <h3 class="titulo_informe"><center><strong>INFORME DE ENERO</strong></center></h3><br>
...
// Se insertan los datos en la DDBB
        $conexion->query("INSERT INTO enero_s (publicaciones, videos, horas, revisitas, cursos, comentarios) VALUES ('$pubs', '$videos', '$hours', '$revisit', '$courses', '$comments')");

// Mensaje de registro exitoso
        echo "<script>alert('INFORME REGISTRADO')</script>";

// Se redirecciona al index del apartado mediante HTML5
